...
Remote module commands use ValkyrAI's existing fenced `Run` contract; remote engine commands use immutable Workflow version snapshots and existing WorkflowExecution lease/fence/idempotency conventions. The bridge validates the exact target and advertised tool, renews leases only through same-origin callbacks, streams bounded progress over the SWARM websocket, replays engine completion/failure events after local restart, and persists terminal GrayMatter receipts and handoffs. A stale fence cannot mutate current execution state. OpenClaw research/drafting is installed as an internal-output capability separate from outbound GTM. Runtime capability remains separate from authority: the node can suspend and resume an approved action, but it can never approve its own request; outbound sends, production deployments, merges, and supervised service restarts remain human-gated.

The optional `service.lifecycle.status` and protected `service.lifecycle.restart` capabilities are discovered from real node-owned launchd/systemd definitions. Commands use only semantic handles for supervised Codex, OpenClaw, ValorIDE SWARM bridge, shared bridge, and Workflow services; caller-selected process names, paths, labels, arguments, shells, and GUI processes are rejected. Claude Code is restartable only when an explicit supervised bridge exists. Shared-bridge self-restart uses a durable private recovery checkpoint and reports completion only after reconnect, native supervisor health, a fresh expected heartbeat/version/capability proof, and a terminal GrayMatter receipt.

Shared-bridge recovery first saves its observed success or failure and exact command binding in the private checkpoint. Completion waits for a persisted GrayMatter MemoryEntry UUID. Queued receipts continue through the existing GrayMatter replay worker; heartbeat reconciliation never submits a second receipt. Replay verifies the saved receipt content before completing recovery, and retains an acknowledged server identity if local completion bookkeeping fails. Queued, degraded, persisted without a valid identity, and persisted pending delivery remain distinct. Invalid, linked, oversized or mismatched checkpoints cannot produce completion.

A crash during the initial receipt submission may leave `submission_started`; a successful response without a usable identity remains unresolved. These require exact source-identity readback or a server-enforced idempotent receipt write before automatic resubmission is safe. The current memory write alias creates an entry and stores `sourceMessageId`; that field alone does not enforce deduplication. This change preserves such uncertainty and does not claim exactly-once remote writes or supervisor command delivery.

The existing receipt replay queue now durably marks a submission before its POST. It can retry a definitive authentication, authorization, or funding rejection (401/403/402). Transport loss, server errors, or a crash after submission stay `outcome_uncertain` until reconciled; a confirmed server UUID survives a later delivery failure. This avoids treating an uncertain paid write as a safe retry.

Preview a shared-bridge artifact update from the candidate package with `node scripts/swarm-service.mjs plan-update --config /path/to/agent.json`. The read-only JSON preview lists every configured agent affected and binds the exact configuration bytes, current native definition, candidate definition, and fixed runtime dependency closure with SHA-256 hashes. It rejects missing or linked definitions, changed host identity, ambiguous agents, and oversized inputs. Repeating an unchanged preview yields the same plan digest. It reads no secure-store values and emits no raw configuration or service definition. Native process state remains `not-observed`; use the canonical status action for health evidence.

The preview does not stage, replace, restart, dispatch, or authorize anything. A governed artifact-update operation must still revalidate the plan against current files, obtain a separately correlated approval for the exact replacement and shared-agent impact, preserve rollback, and verify fresh runtime/heartbeat/terminal-receipt evidence. The existing restart command continues to restart the installed artifact; a preview digest cannot be used as its approval reference.

Prepare the reviewed candidate locally with `node scripts/swarm-service.mjs stage-update --config /path/to/agent.json --expected-plan-sha256 <planSha256>`, using the same package and options as the preview. This writes only to the node-owned `~/.local/share/valkyr-swarm/service-update-candidates/<planSha256>` directory. It saves the plan, candidate native definition, and fixed runtime closure as private read-only files, verifies their hashes, flushes them to disk, rechecks the current configuration/definition/source, and publishes the directory with a same-filesystem rename. Existing candidates are verified on every reuse; stale plans, extra files, links, unsafe permissions, and changed bytes fail closed. The parent directory remains owner-writable for macOS rename compatibility. Read-only permissions prevent accidental edits, not changes by the account owner.

Staging leaves the installed runtime, native service definition, and running process untouched. Its receipt explicitly reports `staged: true`, `applied: false`, and `authorized: false`. A failed or interrupted preparation may leave a private `.preparing-*` directory; a failure after rename may leave an intact candidate for an obsolete plan. Neither is an activation receipt. No update capability is advertised: exact server approval, activation-time plan/artifact verification, durable replacement recovery, rollback, and live health proof remain prerequisites for a future governed update action.
